		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT) {
			...
			if (animations->isEnabled()) {
				...
				if (animations->shouldBlock && g_Data.canUseMoveKeys() && g_Data.isInGame() && !clickGUI->isEnabled()) {
					...
					// 1.7+
					if (animations->mode.getSelectedValue() == 5) {
						lerpT = 0.f;
						matrix = glm::translate<float>(matrix, glm::vec3(0.42222223281, 0.0, -0.16666666269302368));
						matrix = glm::translate<float>(matrix, glm::vec3(-0.1f, 0.15f, -0.2f));
						blockRotate(matrix, 0.25f);
					...
					// Exhi
					if (animations->mode.getSelectedValue() == 6) {
						lerpT = 0.f;
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	if (strcmp(screenName.c_str(), "nether_loading_progress_screen - nether") == 0) {
		...
		if (team->isEnabled() && team->mode.getSelectedValue() == 1) team->RemoveTeams = true;
	}
	...
	if (disabler->isEnabled()) {
		...
		/*if (packet->isInstanceOf<C_MovePlayerPacket>()) {
			if (disabler->mode.getSelectedValue() == 5) {
				...
				actionPacket->pos = g_Data.getLocalPlayer()->currentPos;
				actionPacket->sound = 32;
				//return;
			}
		}*/
...
void Hooks::chatLogHookFunc(__int64 a1, TextHolder* msg, uint32_t a2) {
	...
	if (message.getText() != nullptr && player != nullptr) {
		...
		// AutoQueue
		if (autoHive->isEnabled()) {
			if (autoHive->autoQueue) {
				if (textStr.find("Game OVER!") != string::npos)  autoHive->sendcommand = true;
			}
			// other servers
		}
		...
		if (killsults->isEnabled()) {
			if (textStr.find(name) != string::npos && textStr.find("killed") != string::npos || textStr.find(name) != string::npos && textStr.find("slain") != string::npos)//modosita
...
void Hooks::setPos(C_Entity* ent, vec3_t& pos) {
	...
	static auto detector = moduleMgr->getModule<Detector>();
	static auto flight = moduleMgr->getModule<Flight>();
	static auto killaura = moduleMgr->getModule<Killaura>();
	static auto speed = moduleMgr->getModule<Speed>();

	if (noLagBack->isEnabled() && g_Data.canUseMoveKeys() && g_Data.isInGame() && ent == g_Data.getLocalPlayer() || detector->isEnabled() && detector->nolagback && g_Data.canUseMoveKeys() && g_Data.isInGame() && ent == g_Data.getLocalPlayer()) return;
	if (speed->isEnabled() && speed->mode.getSelectedValue() == 11) speed->clientmessage = true;
	if (detector->disablekillaura && detector->isEnabled()) killaura->setEnabled(false);
	if (detector->disablefly && detector->isEnabled()) flight->setEnabled(false);
	if (detector->disablespeed && detector->isEnabled()) speed->setEnabled(false);
	if (detector->isEnabled()) detector->isnotification = true;
	if (detector->disabledetector) detector->setEnabled(false);
	func(ent, pos);
}
