...
void DrawUtils::drawSystemImage(std::string filePath, vec2_t& imagePos, vec2_t& imageDimension, vec2_t& idk, vec2_t& idk2, MC_Color flushColor) {
	texturePtr = new C_TexturePtr();
	C_FilePath imagepath = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\PacketSkid\\Assets\\") + std::string(filePath);
...
void DrawUtils::drawNameTags(C_Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		DrawUtils::drawGlow(rectPos2, MC_Color(0, 0, 0), 0.05, 15, 4);
...
void DrawUtils::drawDisable(vec4_t(pos)) {
	// line 1
	DrawUtils::fillRectangleA(vec4_t(pos.x, pos.y, pos.x + 30.f, pos.y + 30.5f), MC_Color(255, 0, 0, 255));
	DrawUtils::fillRectangleA(vec4_t(pos.x + 26.f, pos.y, pos.x + 3.5f, pos.y + 30.5f), MC_Color(255, 0, 0, 255));
}
...
/*void DrawUtils::drawRoundRectanglee(vec4_t pos, const MC_Color col, float alpha) {
	DrawUtils::drawBottomLine(vec4_t{ pos.x + 1.5f, pos.y - 2, pos.z - 1.5f, pos.w + 0.5f }, col, alpha);
	DrawUtils::drawQuad({ pos.x, pos.w }, { pos.z, pos.w }, { pos.z, pos.y }, { pos.x, pos.y });
	DrawUtils::fillRectangle(vec4_t(pos.x + 1, pos.y - 1, pos.z - 1, pos.y), col, alpha);
	DrawUtils::setColor(col.r, col.g, col.b, alpha);
}*/
